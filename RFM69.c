...
int RFM69_send(char* data, unsigned int dataLength, uint16_t nodeId) {
	...
	if (true == _csmaEnabled) {
		...
		writeRegister(REG_PACKETCONFIG2,
				(readRegister(REG_PACKETCONFIG2) & 0xFB) | RF_PACKET2_RXRESTART); // avoid RX deadlocks
	...
	while ((readRegister(REG_IRQFLAGS1) & RF_IRQFLAGS1_MODEREADY) == 0x00)
		; // wait for ModeReady
	...
	//while (readRegister(REG_IRQFLAGS2) & RF_IRQFLAGS2_PACKETSENT == 0x00);
...
void RFM69_getVersion(void) {
	char buf[20];
	uint8_t regVal;

	regVal = readRegister(0x10);
